...
// @Success 200 {object} middleware.Response{data=dto.ServiceInfoOutput} "success"
...
func (service *ServiceController) ServiceInfoList(c *gin.Context) {
	...
	for _, info := range infoList {
		...
		if err2 != nil {
			middleware.ResponseError(c, 20003, err2)
			return
		}
		...
		if err != nil {
			middleware.ResponseError(c, 20004, err)
			return
		}
...
func (s *ServiceController) ServiceAddHTTP(c *gin.Context) {
	...
	if len(strings.Split(params.IpList, ",")) != len(strings.Split(params.WeightList, ",")) {
			middleware.ResponseError(c, 20004, errors.New("IP列表与权重列表数量不一致"))
			return
	}
	...
	if err == nil {
		fmt.Println(tmp)
		tx.Rollback()
		middleware.ResponseError(c, 20002, errors.New("服务已存在??"))
		return
	}
	...
	if _, err := httpUrl.Find(c, tx, httpUrl); err == nil {
		tx.Rollback()
		middleware.ResponseError(c, 20003, errors.New("服务接入前缀或域名已存在"))
		return
	}
	...
	if err := serviceModel.Save(c, tx); err != nil {
		tx.Rollback()
		middleware.ResponseError(c, 20005, err)
		return
	}
	//serviceModel.ID
	...
	if err := httpRule.Save(c, tx); err != nil {
		tx.Rollback()
		middleware.ResponseError(c, 20006, err)
		return
	}
	...
	if err := accessControl.Save(c, tx); err != nil {
		tx.Rollback()
		middleware.ResponseError(c, 20007, err)
		return
	}
	...
	if err := loadbalance.Save(c, tx); err != nil {
		tx.Rollback()
		middleware.ResponseError(c, 20008, err)
		return
	}
	tx.Commit()
	middleware.ResponseSuccess(c, "")
...
func (service *ServiceController) ServiceUpdateHTTP(c *gin.Context) {
	...
	if len(strings.Split(params.IpList, ",")) != len(strings.Split(params.WeightList, ",")) {
		middleware.ResponseError(c, 20001, errors.New("IP列表与权重列表数量不一致"))
		return
	}
	...
	if err != nil {
		tx.Rollback()
		middleware.ResponseError(c, 20003, errors.New("服务不存在"))
		return
	}
	serviceDetail, err := serviceInfo.ServiceDetail(c, tx, serviceInfo)
	if err != nil {
		tx.Rollback()
		middleware.ResponseError(c, 20004, errors.New("服务不存在"))
		return
	}
...
func (s *ServiceController) ServiceAddTCP(c *gin.Context) {
	...
	if _, err = serviceInfo.Find(c, tx, serviceInfo); err == nil {
		middleware.ResponseError(c, 20002, errors.New("服务名已存在"))
		return
	}

	TcpUrl := &dao.TcpRule{Port: params.Port}
	if _, err := TcpUrl.Find(c, tx, TcpUrl); err == nil {
		middleware.ResponseError(c, 20003, errors.New("端口被占用"))
		return
	}
