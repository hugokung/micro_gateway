...
type ServiceInfoInput struct {
	...
	PageSize	int			`json:"page_size" form:"page_size" comment:"页面大小" example:"3" validate:"required"`
}

func (serviceInput *ServiceInfoInput) BindValidParam(c *gin.Context) error {
	return public.DefaultGetValidParams(c, serviceInput)
}

type ServiceInfoItem struct {
	ID				int64			`json:"id" form:"id" comment:"服务id" example:"1" validate:""`
	LoadType		int				`json:"load_type" form:"load_type" comment:"加载类型" example:"1" validate:""`
	ServiceName		string			`json:"service_name" form:"service_name" comment:"服务名称" example:"test_grpc" validate:""`
...
type ServiceInfoOutput struct {
	...
	InfoList	[]*ServiceInfoItem	`json:"info_list"`
}
